...
const TagPlayersWhenUploadComponent = ({
  onSubmit,
  isOpen,
  setOpenModal,
}: TagPlayersWhenUploadComponentProps) => {
  ...
  const accountsData = useMemo(() => {
    ...
    return getAccountsSelect.data.map((account: IUserData) => ({
      ...
      label:
        getMe.data?.id.toString() == account.id.toString()
          ? `${account.username} (Me)`
          : account.username,
    })) as ComboboxData;
  ...
  const form = useForm({
    ...
    validate: {
      topPlayer: (value) => {
        if (
          !accountsData.some((item) => (item as ComboboxItem).value === value)
        ) {
        ...
        if (
          value !== getMe.data?.id.toString() &&
          form.values.bottomPlayer !== getMe.data?.id.toString()
        ) {
      ...
      bottomPlayer: (value) => {
        ...
        if (
          value !== getMe.data?.id.toString() &&
          form.values.topPlayer !== getMe.data?.id.toString()
        ) {
          return "Either top player or bottom player must be you";
        }
        return null;
  ...
  return (
    ...
    >
      ...
      >
        <Select
          key={form.key("topPlayer")}
          {...form.getInputProps("topPlayer")}
          required
          name="topPlayer"
          label="Top Player"
          data={accountsData}
        />
        <Select
          key={form.key("bottomPlayer")}
          {...form.getInputProps("bottomPlayer")}
          required
          name="bottomPlayer"
          label="Bottom Player"
          data={accountsData}
