...
class PlayerPoseExtractor:
    ...
    def extract_pose(self, image, player_boxes):
        ...
        if len(player_boxes) > 0:
            margin = 50
            xt, yt, xb, yb = player_boxes
            xt, yt, xb, yb = int(xt), int(yt), int(xb), int(yb)
            patch = image[max(yt - margin, 0):min(yb + margin, height),
                    max(xt - margin, 0):min(xb + margin, width)].copy()
        else:
            margin = 0
        ...
        stickman[max(yt - margin, 0):min(yb + margin, height),
        max(xt - margin, 0):min(xb + margin, width)] = patch_zeros
    ...
    def save_to_csv(self, output_folder, player_id=1):
        ...
        outfile_path = os.path.join(output_folder, 'stickman_data_' + str(player_id) + '.csv')
