...
class Smooth:
    ...
    def smooth(self, df=None):
        ...
        if df is not None:
            ...
            for i in range(len(df.columns)):
                col_data = df.iloc[:, i].dropna()  # Remove NaNs before filtering
                if len(col_data) > 2:  # Only apply filter if there are enough points
                    window_length = min(self.window_length, len(col_data))
                    df.iloc[:, i] = signal.savgol_filter(col_data, window_length, self.polyorder)
    ...
    def save_to_csv(self, output_folder, player_id=1):
        """
        Saves the data frames after smoothing
        :param output_folder: str, path to output folder
        """
        if self.df_smooth is not None:
            outfile_path = os.path.join(output_folder, 'stickman_data_smoothed_' + str(player_id) + '.csv')
