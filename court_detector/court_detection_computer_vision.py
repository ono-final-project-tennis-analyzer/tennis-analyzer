...
class CourtDetectorComputerVision:
    ...
    def track_court(self, frame):
        ...
        for line in lines:
            ...
            p1 = None
            p2 = None
            if line[0][0] > self.frame_width or line[0][0] < 0 or line[0][1] > self.frame_height or line[0][1] < 0:
                for p in points_on_line:
                    if 0 < p[0] < self.frame_width and 0 < p[1] < self.frame_height:
                        p1 = p
                        break
            if line[1][0] > self.frame_width or line[1][0] < 0 or line[1][1] > self.frame_height or line[1][1] < 0:
            ...
            for p in points_on_line:
                ...
                bottom_y, bottom_x = min(p[1] + self.dist, self.frame_height), min(p[0] + self.dist, self.frame_width)
            ...
            if len(new_points) < 2:  # If not enough points, skip this line
                continue
            # find line fitting the new points
            [vx, vy, x, y] = cv2.fitLine(new_points, cv2.DIST_L2, 0, 0.01, 0.01)
            new_lines.append(((int(x - vx * self.frame_width), int(y - vy * self.frame_width)),
                              (int(x + vx * self.frame_width), int(y + vy * self.frame_width))))
            ...
            if len(new_points) < 50:
                if self.dist > 20:
                    try:
                        ...
                        conf_points = np.array(self.court_reference.court_conf[self.best_conf],
                        ...
                        print('Failed to track court again, probably the court is not visible')
                        self.success_flag = False
                        self.dist = 5
                        return
                else:
...
if __name__ == '__main__':
    ...
    clay_court_detection = CourtDetectorComputerVision(verbose=False)
    ...
    hard_court_detection = CourtDetectorComputerVision(verbose=False)
    ...
    grass_court_detection = CourtDetectorComputerVision(verbose=False)
